"""
Visualization utilities for Robot Pose Estimation
"""
...
def visualize_sample(sample, pred_keypoints=None, config=None):
    """
    Visualize a dataset sample with ground-truth and optional predicted keypoints.
    
    Args:
        sample: dict from dataset
        pred_keypoints: optional (12,) array of predicted keypoints normalized to crop
        config: config module or dict (for joint names and sizes)
    """
    # Get config values
    if config is None:
        stage1_size = 256
    ...
    # Right: Cropped image with keypoints
...
def visualize_pipeline_result(image_path, pipeline, gt_keypoints=None, config=None):
    """
    Visualize full pipeline prediction on an image.
    
    Args:
        image_path: path to image
        pipeline: TwoStagePipeline object
        gt_keypoints: optional ground truth keypoints (6, 2) or (12,)
...
def visualize_bbox_predictions(dataset, model, device, num_samples=4, config=None):
    """Visualize bbox predictions on full images."""
    if config is None:
        stage1_size = 256
    elif hasattr(config, 'STAGE1_SIZE'):
        stage1_size = config.STAGE1_SIZE
...
def plot_error_distribution(errors, title='Error Distribution'):
    """Plot histogram and sorted errors."""
