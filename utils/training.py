"""
Training utilities for Robot Pose Estimation

Includes robust training loops with:
...
def train_one_epoch_keypoints(model, loader, optimizer, device, img_size=512):
    """Train keypoint model for one epoch."""
    model.train()
...
def train_one_epoch_bbox(model, loader, optimizer, device):
    """Train bbox model for one epoch."""
    model.train()
    loss_meter = AverageMeter()
...
def train_stage2(
    ...
    """
    Train Stage 2 (keypoint) model with robust checkpointing.
    ...
    Args:
        ...
        img_size: image size for pixel error computation
...
def train_stage1(
    ...
    """
    Train Stage 1 (bbox) model with robust checkpointing.
